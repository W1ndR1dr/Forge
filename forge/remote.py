"""
Remote execution for Forge.

This module enables Forge running on a Raspberry Pi to execute
commands on a remote Mac via SSH over Tailscale.

Why this exists:
- Git worktrees need to be created on the Mac where Claude Code runs
- The Forge server can run on a Pi for always-on availability
...
class RemoteExecutor:
    ...
    def run_forge_command(
        ...
        """
        Run a Forge command on the remote Mac.
    ...
    def test_connection(self) -> tuple[bool, str]:
        ...
        Returns:
            (success, message)
        """
        result = self.run_command(["echo", "Forge connection test"], timeout=15)
    ...
    def get_projects(self, projects_base: Path) -> list[dict]:
        """
        List Forge-initialized projects on the remote Mac.
        ...
        """
        # Find directories with .forge
        result = self.run_command(
            ["find", str(projects_base), "-maxdepth", "2", "-name", ".forge", "-type", "d"],
            timeout=30,
        )
