"""
Forge MCP Server - Remote MCP integration for Claude Code.

This module implements a Model Context Protocol (MCP) server that allows
Claude Code (on iOS, web, or anywhere) to natively control Forge.
...
Forge functionality from Claude Code on iPhone.
...
class ForgeMCPServer:
    """
    MCP Server implementation for Forge.
    ...
    def __init__(
        ...
        Args:
            projects_base: Base directory containing Forge projects (Mac path)
    ...
    def _auto_migrate_project(self, project_name: str) -> bool:
        ...
        if not self.remote_executor.dir_exists(forge_dir):
            return False

        # Read registry and config from Mac
        registry_content = self.remote_executor.read_file(forge_dir / "registry.json")
        config_content = self.remote_executor.read_file(forge_dir / "config.json")
    ...
    ) -> tuple[Path, ForgeConfig, FeatureRegistry]:
        """
        ...
        return [
            {
                ...
                "description": "List all Forge-initialized projects",
                "inputSchema": {
                    "type": "object",
                    "properties": {},
                    "required": [],
            ...
            },
            {
                ...
                "description": "Initialize Forge in a project directory. Creates .forge config and enables feature tracking.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "project": {
    ...
    def call_tool(self, tool_name: str, arguments: dict) -> MCPToolResult:
        ...
        tool_handlers = {
            "forge_list_projects": self._list_projects,
            "forge_list_features": self._list_features,
            "forge_status": self._get_status,
            "forge_start_feature": self._start_feature,
            "forge_stop_feature": self._stop_feature,
            "forge_demote_feature": self._demote_feature,
            "forge_merge_check": self._merge_check,
            "forge_merge": self._merge_feature,
            "forge_add_feature": self._add_feature,
            "forge_init_project": self._init_project,
        }
    ...
    def _list_projects(self) -> MCPToolResult:
        """
        List all Forge-initialized projects.
    ...
    def _start_feature(
        ...
        worktree_base = config.project.worktree_base or ".forge-worktrees"
        ...
        prompt_dir = project_path / ".forge" / "prompts"
    ...
    def _smart_done_feature(self, project: str, feature_id: str) -> MCPToolResult:
        ...
        if self._check_mac_online():
            ...
            if branch_merged:
                ...
                worktree_path = project_path / ".forge-worktrees" / feature_id
    ...
    def _cleanup_orphans(self, project: str) -> MCPToolResult:
        ...
        worktree_base = project_path / ".forge-worktrees"
    ...
    def _sync_registry(self, project: str, direction: str = "pi-to-mac") -> MCPToolResult:
        ...
        mac_registry_path = project_path / ".forge" / "registry.json"
    ...
    def _init_project(
        ...
        This operation REQUIRES Mac to be online (creates .forge on Mac).
        ...
        if self.remote_executor.dir_exists(forge_dir):
            return MCPToolResult(
                success=False,
        ...
        if not self.remote_executor.dir_exists(forge_dir):
            return MCPToolResult(
                success=False,
                message="Initialization command succeeded but .forge not created",
            )
        ...
        return MCPToolResult(
            success=True,
            message=f"Initialized Forge in {project}",
            data={
                "project": project,
