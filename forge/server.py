"""
Forge HTTP Server - FastAPI wrapper for MCP and Web UI.
...
from .mcp_server import ForgeMCPServer, create_mcp_response
...
def get_config() -> dict:
    ...
    return {
        ...
        "mac_projects_base": os.environ.get("FORGE_MAC_PROJECTS_PATH"),
        "remote_host": os.environ.get("FORGE_MAC_HOST"),
        "remote_user": os.environ.get("FORGE_MAC_USER"),
...
mcp_server: Optional[ForgeMCPServer] = None
...
async def lifespan(app: FastAPI):
    ...
    print(f"Forge MCP Server started (Pi-native architecture)")
    ...
    print("Forge MCP Server stopped")
    ...
    return {
        ...
        "endpoints": {
            "system": {
                ...
                "GET /api/projects": "List all Forge projects",
                "GET /api/system/status": "System status with Mac connectivity",
            },
            "project": {
                "POST /api/{project}/init": "Initialize Forge in project",
...
class InitProjectRequest(BaseModel):
    """Request to initialize Forge in a project."""
    quick: bool = True
...
async def init_project(project: str, request: InitProjectRequest):
    ...
    - .forge/config.json
    - .forge/registry.json
    - .forge/project-context.md
    - .forge/prompts/ directory
    - .forge/research/ directory
...
async def get_project_health(project: str):
            ...
            if result.success:
                ...
                for line in result.stdout.strip().split("\n"):
                    if line.startswith("worktree "):
                        ...
                        if ".forge-worktrees" in str(current_path):
    ...
    for wt_path in worktree_paths:
        if wt_path not in registry_worktree_paths:
            # Check if it's in the .forge-worktrees directory
            if ".forge-worktrees" in str(wt_path):
                issues.append({
                    "feature_id": None,
...
def get_session_memory():
    ...
    if _session_memory is None:
        ...
        config = get_config()
        data_dir = config["projects_base"] / ".forge-memory"
...
async def brainstorm_websocket(websocket: WebSocket, project: str):
    ...
    try:
        ...
        try:
            ...
            if remote_executor:
                context_path = project_path / ".forge" / "project-context.md"
