# FlowForge macOS App - Quick Start Guide
...
4. FlowForge CLI installed and in PATH
...
1. Navigate to the app directory:
   ```bash
   cd ~/Projects/Active/FlowForge/FlowForgeApp
   ```
...
On first launch, the app will automatically scan these directories for FlowForge projects:
- `~/Projects/Active`
- `~/Projects`
- `~/Developer`

Any directory containing a `.flowforge/` folder will be detected.
...
**Sidebar (Left)**:
- Lists all discovered FlowForge projects
...
- Make sure you have initialized FlowForge in at least one project
- Run `forge init` in a project directory
- Click refresh button in sidebar

### Features Not Appearing
- Check that the project has features in `.flowforge/registry.json`
...
```bash
watch -n 1 cat .flowforge/registry.json
```
