"""
Forge CLI - AI-assisted parallel development orchestrator.
...
app = typer.Typer(
    name="forge",
    help="Forge: AI-assisted parallel development orchestrator",
    no_args_is_help=True,
)
console = Console()


def get_context(require_init: bool = True) -> tuple[Path, ForgeConfig, FeatureRegistry]:
    """Get project context (root, config, registry)."""
    project_root = find_project_root()

    if require_init and not (project_root / ".forge").exists():
        console.print("[red]Forge not initialized. Run 'forge init' first.[/red]")
        raise typer.Exit(1)

    config = ForgeConfig.load(project_root) if require_init else None
...
def init(
    ...
    if (project_root / ".forge").exists():
        if not Confirm.ask("Forge already initialized. Reinitialize?"):
            raise typer.Exit(0)

    console.print(f"\n🔨 Initializing Forge in [cyan]{project_root}[/cyan]\n")
    ...
    # Create config
    config = ForgeConfig(project=detected)
    ...
    (project_root / ".forge" / "prompts").mkdir(parents=True, exist_ok=True)
    (project_root / ".forge" / "research").mkdir(parents=True, exist_ok=True)
    ...
    console.print("\n🎉 Forge initialized! Run [cyan]forge add[/cyan] to add features.")
...
def brainstorm(
    ...
    console.print(Panel(
        ...
        title=f"Forge: {config.project.name}",
    ))
